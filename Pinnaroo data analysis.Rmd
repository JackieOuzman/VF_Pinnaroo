...
I have emailed Gallagher to confrim.
...
For VF 1 - 4 this is easy, when I have the correct fence ID matches in the shapefiles and aniamal movment file.
...
I have also filter out data within a date and time range (not sure about the time)
...
The fence ID in the animal movement file is not the same as the VF boundaries.
...
I am not really sure what fence ID relates to what VF area.
...
Three_pm_per_animal <- ungroup(  Three_pm_per_animal)

# make df narrow


Three_pm_per_animal <- pivot_longer(Three_pm_per_animal,
...
Three_pm_per_animal_table <- Three_pm_per_animal %>% 
  filter(stimuli != "ratio" ) %>%
  group_by(date, stimuli)%>% 
  summarise(
    mean_stimuli = mean(value   , na.rm =TRUE))
Three_pm_per_animal_table
...
Three_pm_per_animal_summary_per_day <- Three_pm_per_animal %>% 
  group_by(date, stimuli)%>%
  summarise(stimuli_av_day = mean(value, na.rm = TRUE))



Three_pm_per_animal_summary_per_day %>%
  filter(stimuli == "ratio" ) %>% 
...
Three_pm_per_animal_summary_per_day_table <- Three_pm_per_animal_summary_per_day %>% 
  filter(stimuli == "ratio" ) %>%
  group_by(date, stimuli)%>% 
  summarise(
    mean_stimuli = mean(stimuli_av_day   , na.rm =TRUE))
Three_pm_per_animal_summary_per_day_table
